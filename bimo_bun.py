...
class StressAndEmotionAnalyzer:
    ...
    def analiza_cadru(self, frame):
        ...
        if len(faces) > 0:
            ...
            if self.ultim_cx is not None:

                dist = ((cx - self.ultim_cx) ** 2 +
                        (cy - self.ultim_cy) ** 2) ** 0.5

                
                dist_normalizat = dist / w

                if dist_normalizat > 0.12:
        ...
        if timp_curent - self.timp_ultima_scanare >= 5:
            ...
            try:
                ...
                if procente["happy"] > 60:
                    emotie_curenta = "neutral"

                elif procente["angry"] > 30:
                    emotie_curenta = "angry"

                elif procente["fear"] > 30:
                    emotie_curenta = "fear"

                elif procente["sad"] > 30:
                    emotie_curenta = "sad"
                ...
                try:
                    self.ultima_emotie_cunoscuta = statistics.mode(
                        self.istoric_emotii
                    )
                except:
        ...
        return (
            self.ultima_emotie_cunoscuta,
            self.nivel_stres_fizic,
            self.agitatie_cap
        )
    ...
    def _deschide_microfon(self):
        ...
        for incercare in range(5):
            try:
                ...
                print(f"❌ [EROARE AUDIO CRITICĂ la deschidere]: {e}")
                time.sleep(0.5)
        self.running = False
        return False
    ...
    def run(self):
        while self.running:
            ...
            try:
                ...
                #resampled_audio = scipy.signal.resample_poly(audio_data, 160, 441).astype(np.int16)
    ...
    compute_type="int8"
            ...
            if self.res.model is None:
                try:
                    ...
                    self.res.model = WhisperModel("base", device="cpu", compute_type="int8")
                except Exception as e_load:
            ...
            try:
                ...
                with open("fallback_offline.wav", "wb") as f:
                    f.write(audio.get_wav_data())
                
                
                segments, _ = self.res.model.transcribe("fallback_offline.wav", language="ro")
                text_intrebare = "".join([segment.text for segment in segments])
                
                
               
                if "oprire" in text_intrebare.lower() or "stop" in text_intrebare.lower():
                    self.res.dialog.put("OPRIRE_SISTEM")
                elif "ce vezi" in text_intrebare.lower():
                    self.res.dialog.put("CE_VEZI")
                else:
...
class BiometricMonitor(threading.Thread):
    ...
    "https://bimo-companion.base44.app/api/functions/bimoSettings",
    ...
    def run(self):
        self.res.puls_curent=int(self.baseline)
            ...
            try:
                ...
                if cadru_curent is not None:
                    ...
                    if persoana_detectata:
                        emotie, stres_fizic, agitatie = self.analyzer.analiza_cadru(cadru_curent)
    ...
    f"Agitatie={self.analyzer.agitatie_cap}"
                        ...
                        if timp_actual - self.ultimul_update_baseline >= 60:
                            ...
                            self.settings=self.primeste_date()
                            if self.settings["silent_mode"]:
                                print("silentios=on")
                            elif self.settings["speak_mode"]:
                                print("silentios=off")
...
def main():
    ...
    biometric_monitor.settings["speak_mode"] = False
    sfat_curent_ales = ""
    timp_start_sfat = 0
    exercising = False
    ...
    while running:
        ...
        utils.vorbire=biometric_monitor.settings["speak_mode"]
        ...
        if not res.dialog.empty():
            ...
            if mesaj == "WAKE_WORD_DETECTAT":
                ...
                #threading.Thread(target=bimo.dezabrobare).start()
                ui.loading_screen(ecran, "Oprire sistem", 3, mijloc_x, mijloc_y)
